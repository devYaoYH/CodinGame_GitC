...
def scoreBomb(tgtID, curID, simulStates):
    targetStates = simulStates[tgtID]
    ttt = adjMatrix[curID][tgtID]
    tttState = targetStates[ttt]
    distanceMultiplier = (1/max(1,(ttt**0.5)))
    score = 0
...
def should_bomb(tgtID, curID, simulStates):
    targetStates = simulStates[tgtID]
    ttt = adjMatrix[curID][tgtID]
    tttState = targetStates[ttt]
    if (tttState.owner != -1):
...
while True:
    ...
    currentSimulation = [fac.resolve() for fac in factoryInfo]
    ...
    print("Bombs Left: {0} | Frontline Factory {1}".format(num_bombs, FRONTLINE_FACTORY), file=sys.stderr)
    # Launch BOMBS!
    if (num_bombs > 0 and FRONTLINE_FACTORY != -1):
        print("Attempting BOMB", file=sys.stderr)
        # Scores all enemy factores for bombing! :D
        bombTargets = [(fac.ID, scoreBomb(fac.ID, FRONTLINE_FACTORY, currentSimulation)) for fac in factoryInfo if (should_bomb(fac.ID, FRONTLINE_FACTORY, currentSimulation))]
    ...
    if (SIMULATE_ENEMY):
        ...
        for enemyFac in enemies:
            ...
            enemyActions.extend(simulateEnemySmart(enemyFac, currentSimulation))
    ...
    enemySimulation = [fac.resolve() for fac in factoryInfo]

    # Feed Strategizer
    strategize = Strategizer(enemySimulation, simulFac, turnBombs, turnIncs, simulIDCounter)
