...
def readMaxAvailTroops(simulStates):
    ...
    if (simulStates[-1].owner == -1): # Overrun by enemy
        ...
        for state in simulStates:
            ...
            # turn += 1
            if (state.owner != 1):
                availTroops = -state.owner
                ttt = turn
                break
            elif (state.owner == 1):
                if (state.troops < availTroops):
                    availTroops = state.troops
                    ttt = turn
            turn += 1
...
def getValidTargets(curID, curBlacklist, simulStates): # Returns list of Factory objects
    ...
    for adj in adjList[curID]:
        ...
        # Ignores neutral non-production factory
        if (targetStates[ttt].owner == 0 and targetStates[ttt].production == 0):
            print("IGNORE -> Neutral Non-production", file=sys.stderr)
            ignore = True
...
def scoreBomb(tgtID, curID):
    distanceMultiplier = (1/max(1,(adjMatrix[curID][tgtID]**0.5)))
    score = 0
    score += factoryInfo[tgtID].production*PRODUCTION_MULTIPLIER*distanceMultiplier # Rewards production
...
class Factory(object):
    ...
    def reinforce(self, simulStates): #TODO: How to bring troops to the front?
        ...
        for target in adjMyFactories:
            weightedTargets.append((target, scoreRedistribution(target, self.ID, closestEnemy(factoryInfo[target])[1])))
        weightedTargets = sorted(weightedTargets, key=lambda x: x[1], reverse=True)
    ...
    def attack(self, simulStates): #TODO: Where to upgrade factories??
        ...
        for targetFac in validTargets:
            ...
            if (targetState.troops == 0 and targetState.owner == 0 and targetState.production > 0):
        ...
        weightedTargets = []
        for targetFac in validTargets:
            # Ignore targets with <0 score
            targetScore = scoreTarget(targetFac.ID, self.ID)
            if (targetScore < 0):
                continue
            weightedTargets.append((targetFac, targetScore))
        weightedTargets = sorted(weightedTargets, key=lambda x: x[1], reverse=True)
        ...
        for targetTup in weightedTargets:
            ...
            ttt = floydWarMatrix[self.ID][targetFac.ID]+1
            tttState = targetStates[ttt]
            print("Testing Priority attack: {0} | ttt: {1} | tttState: Owner->{2} Troops->{3}".format(targetFac.ID, ttt, tttState.owner, tttState.troops), file=sys.stderr)
            ...
            if (targetAttack):
                ...
                print("Overwhelming target {0}".format(targetFac.ID), file=sys.stderr)
                print(self.actions[-1].print(), file=sys.stderr)
                curTroops -= targetTroops
...
class Strategizer(object):
    ...
    def prune(self): #TODO: prunes excess troops sent and orgnize co-ordinated attacks
        ...
        for targetTup in weightedTargets:
            ...
            adjFacsID = [adjTup[0] for adjTup in adjList[target.ID]]
            availFacs = [facTup for facTup in facTroopList if (facTup[1] in adjFacsID)]
            # Simulates attack
    ...
    def whack(self): #TODO: Whacks enemy with all we've got
        ...
        for fac in myFactories:
            if (fac.production != 3): # We do not whack from non-fully upgraded factories
                continue
            curTroops = fac.reportAvailTroops(self.resolutions)
